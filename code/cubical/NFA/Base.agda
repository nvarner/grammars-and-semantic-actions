...
record NFA : Type (ℓ-suc ℓN) where
  ...
  module Trace (q-end : ⟨ Q ⟩) where
    ...
    ftor q-start = ⊕e Tag λ where
      ...
      cons → ⊕e ⟨ transition ⟩ λ tr → ⊕e (src tr Eq.≡ q-start) λ _ →
        ⊗e (k (LiftGrammar (literal (label tr)))) (Var (dst tr))
      ε-cons → ⊕e ⟨ ε-transition ⟩ λ εtr → ⊕e (ε-src εtr Eq.≡ q-start) λ _ →
        Var (ε-dst εtr)

    Trace : (q-start : ⟨ Q ⟩) → Grammar ℓN
    Trace = μ ftor
    ...
    CONS : ∀ tr →
      literal (label tr) ⊗ Trace (dst tr) ⊢ Trace (src tr)
    CONS tr = roll ∘g LinΣ-intro cons ∘g LinΣ-intro tr ∘g LinΣ-intro Eq.refl ∘g Lift-intro ,⊗ id

    ε-CONS : ∀ εtr →
      Trace (ε-dst εtr) ⊢ Trace (ε-src εtr)
    ε-CONS εtr = roll ∘g LinΣ-intro ε-cons ∘g LinΣ-intro εtr ∘g LinΣ-intro Eq.refl
